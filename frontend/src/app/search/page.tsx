...
function ProductCard({ item, type }: { item: any, type: string }) {
  ...
  if (type === 'hotel') {
    ...
    features = ['🍃 100% Renewable Energy', '🚫 Single-use plastic free'];
  } else if (type === 'bus') {
    ...
    features = ['⚡ Zero Emissions', '💺 Extra Legroom'];
  } else if (type === 'tour') {
    title = item.title;
    ...
    features = ['🌍 Carbon Neutral', '🚶 Local Guide'];
  } else if (type === 'flight') {
    title = `${item.origin} to ${item.destination}`;
    subtitle = `${item.airline} • ${item.flightNumber}`;
    price = item.price;
    image = 'https://images.unsplash.com/photo-1436491865332-7a61a109cc05?auto=format&fit=crop&w=1200&q=80';
    features = ['SAF Fuel Used', '🌳 Offset Included'];
  }
